...
```bash
git clone https://github.com/mdtsai456/Life-Course-REMBG.git
cd Life-Course-REMBG
```

### 啟動

在專案根目錄執行以下指令。會同時啟動 **backend**（去背 API）與 **web**（前端網站 + Nginx 反向代理）。
...
- 網站：`http://localhost:8080/`（若將 compose 埠改為 `80:8080`，則為 `http://localhost/`）。
- 健康檢查：`http://localhost:8080/health`（同上埠映射時為 `http://localhost/health`）。
